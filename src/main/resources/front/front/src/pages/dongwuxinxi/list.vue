...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","margin":"0 auto","borderColor":"#ffc800","alignItems":"center","display":"flex","borderRadius":"0 0 12px 12px","flexWrap":"wrap","background":"#fbf9f3","borderWidth":"0 1px 1px 1px","width":"1200px","borderStyle":"solid","height":"auto","order":"2"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>动物名称</div>
        <el-input v-model="formSearch.dongwumingcheng" placeholder="动物名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>动物性别</div>
        <el-select v-model="formSearch.dongwuxingbie" placeholder="请选择动物性别" :clearable="true">
          <el-option v-for="(item, index) in dongwuxingbieOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('dongwuxinxi','新增')" :style='{"cursor":"pointer","border":"1px solid #db961f","padding":"0px 15px","boxShadow":"1px 1px 3px #f8a412","margin":"0px 10px 0 0","color":"#333","outline":"none","borderRadius":"4px","background":"linear-gradient(180deg, rgba(255,219,161,1) 0%, rgba(255,219,161,1) 51%, rgba(252,188,69,1) 51%, rgba(252,188,69,1) 100%)","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}' type="primary" @click="add('/index/dongwuxinxiAdd')"><i v-if="true" :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"width":"100%","margin":"0","background":"none","order":"3"}'>
		...
		<div class="list1 index-pv1" :style='{"border":"1px solid #ffc800","padding":"30px 20px 20px","boxShadow":"0px 0px 0px #ddd,inset 0px 0px 30px 0px #fbf5db","margin":"20px 0 0 0","overflow":"hidden","borderRadius":"16px","flexWrap":"wrap","background":"#fff","display":"block","width":"calc(100% - 2px)","justifyContent":"space-around","height":"auto"}'>
			<div :style='{"margin":"0 0 30px 2%","borderRadius":"8px","flexWrap":"wrap","background":"none","display":"flex","width":"31%","position":"relative","float":"left","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"cursor":"pointer","padding":"8px","borderColor":"#ffa100","margin":"0 0 10px","objectFit":"cover","borderRadius":"20%","borderWidth":"3px","background":"#f5f6cb","display":"block","width":"100%","borderStyle":"dotted dashed solid double","height":"274px"}' v-if="item.dongwuzhaopian && item.dongwuzhaopian.substr(0,4)=='http'" :src="item.dongwuzhaopian" class="image" />
				<img :style='{"cursor":"pointer","padding":"8px","borderColor":"#ffa100","margin":"0 0 10px","objectFit":"cover","borderRadius":"20%","borderWidth":"3px","background":"#f5f6cb","display":"block","width":"100%","borderStyle":"dotted dashed solid double","height":"274px"}' v-else :src="baseUrl + (item.dongwuzhaopian?item.dongwuzhaopian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"0 10px","boxShadow":"0px 2px 0px #f1e288","margin":"0 auto 10px","whiteSpace":"nowrap","color":"#333","textAlign":"center","overflow":"hidden","borderRadius":"20% ","background":"#f6f6f6","width":"76%","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">{{item.dongwumingcheng}}</div>
			</div>
		</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
        this.$http.get('option/dongwuzhonglei/dongwuzhonglei').then(res => {
          if (res.data.code == 0) {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.dongwumingcheng != '') searchWhere.dongwumingcheng = '%' + this.formSearch.dongwumingcheng + '%';
        if (this.formSearch.dongwuxingbie != '') searchWhere.dongwuxingbie = this.formSearch.dongwuxingbie;
        if (this.curFenlei != '全部') searchWhere.dongwuzhonglei = this.curFenlei;
        this.$http.get('dongwuxinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/dongwuxinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
