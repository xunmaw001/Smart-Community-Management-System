	...
	<div class="addEdit-block" :style='{"padding":"30px 5%"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="select" v-if="type!='info'"  label="房屋号" prop="fangwuhao">
					<el-select :disabled="ro.fangwuhao" v-model="ruleForm.fangwuhao" placeholder="请选择房屋号" >
						<el-option
	...
	methods: {
		...
		init(id,type) {
            ...
            this.$http({
				...
				if (data && data.code === 0) {
					this.fangwuhaoOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
